...
def navigate(
    ...
    with sync_playwright() as p:
        ...
        try:
            ...
            # Bug 1 fix: capture post-redirect URL as the real start
            actual_start = normalize_url(nav_page.url)
            ...
            for step in range(config.MAX_STEPS):
                ...
                # ── Guard: already visited (start URL exempt) ─────────
                is_start = (cur_norm == actual_start)
...
def _try_click(page, elements: list[dict], element_id: int) -> bool:
    ...
    try:
        ...
        if target_url and not target_url.startswith("javascript:"):
            ...
            safe_text = chosen["text"].replace("\\", "\\\\").replace('"', '\\"')
            try:
                page.locator(f'text="{safe_text}"').first.click(timeout=5000)
            except Exception:
                # Bug 4 fix: filter to visible elements only, matching extraction script
                page.evaluate(
                    "(idx) => { const visible = Array.from("
                    "document.querySelectorAll('a,button,[role=button]'))"
                    ".filter(el => el.offsetParent !== null);"
                    "if (visible[idx]) visible[idx].click(); }",
                    element_id,
                )
...
def _parse_decision(raw: str) -> int | str | None:
    ...
    return int(numbers[0]) if numbers else None    # Bug 3 fix: first number = element id
